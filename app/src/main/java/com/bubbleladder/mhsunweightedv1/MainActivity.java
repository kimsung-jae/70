...
public class MainActivity extends Activity{
    ...
    private View buildUi(){
        ...
        root.addView(tv("보글사다리 · AUTO FIND V1.3",24,Color.WHITE,true));TextView sub=tv("PASS 없음 · 약한 MHS면 다른 방법 자동탐색 · 매회 최종1픽 1개",12,Color.rgb(110,231,183),false);sub.setPadding(0,dp(4),0,dp(14));root.addView(sub);
        ...
        LinearLayout ctrl=card();refresh=button("🔄 지금 수집 / 방법 자동탐색",Color.rgb(5,150,105));ctrl.addView(refresh,new LinearLayout.LayoutParams(-1,dp(54)));background=new CheckBox(this);background.setText("백그라운드 자동추첨 ON");background.setTextColor(Color.WHITE);background.setTextSize(15);background.setPadding(0,dp(8),0,0);ctrl.addView(background);status=tv("조회 준비",12,Color.rgb(203,213,225),false);status.setPadding(0,dp(6),0,0);ctrl.addView(status);root.addView(ctrl);
        LinearLayout hero=card();hero.addView(tv("다음 회차 · AUTO FIND 최종 1픽",12,Color.GRAY,false));nextRound=tv("-",15,Color.WHITE,true);hero.addView(nextRound);bestPick=tv("분석 대기",30,Color.rgb(52,211,153),true);bestPick.setPadding(0,dp(7),0,0);hero.addView(bestPick);bestConf=tv("약하면 다른 분석법으로 재탐색 · 매회 추천",15,Color.rgb(253,224,71),true);bestConf.setPadding(0,dp(4),0,0);hero.addView(bestConf);TextView note=tv("MHS → Markov차수/N-gram/Shape/kNN/Run/Lag/Regime/역방향까지 재탐색",12,Color.rgb(110,231,183),true);note.setPadding(0,dp(10),0,0);hero.addView(note);root.addView(hero);
        ...
        LinearLayout pc=card();pc.addView(section("AUTO FIND 재탐색 엔진 상세"));patternDetail=tv("-",13,Color.rgb(226,232,240),false);patternDetail.setLineSpacing(0,1.3f);pc.addView(patternDetail);root.addView(pc);
        ...
        root.addView(tv("※ 표시 %는 선택된 방법의 과거 walk-forward 적중률입니다. 미래 적중을 보장하지 않습니다. 70% 미달이어도 대기하지 않고 다른 방법을 재탐색해 1픽을 추천합니다.",11,Color.GRAY,false));return sv;
    }
    ...
    private void manualSync(){
        saveSettingsSilent();refresh.setEnabled(false);status.setText("API 수집 + 자동 방법탐색 중...");
    ...
    private void render(FlowCore.Analysis a,List<FlowCore.Result>d){if(a==null||d==null||d.isEmpty())return;FlowCore.Result last=d.get(0);nextRound.setText(last.round<480?last.date+" · "+(last.round+1)+"회":"다음날 · 1회");
        ...
        if(t!=null&&t.certified){
            String badge=t.targetAchieved?"🔥 70+ 탐색 성공":"🔎 강제 자동탐색 추천";
            bestConf.setText(badge+" · 검증 "+FlowCore.pct(t.validationRate)+(t.validationN>0?" · "+t.validationHit+"/"+t.validationN:" · 초기표본"));
            bestPick.setTextColor(t.targetAchieved?Color.rgb(52,211,153):Color.rgb(253,224,71));
        }else{bestConf.setText("자동탐색 준비중");bestPick.setTextColor(Color.rgb(253,224,71));}
        ...
        StringBuilder ps=new StringBuilder();if(t==null){ps.append("AUTO FIND 엔진 준비중");}else{ps.append(t.detail).append("\n\n탐색한 합성규칙: ").append(t.searched).append("개");if(t.certified){ps.append("\n선택 항목: ").append(FlowCore.DIM[t.dim]).append(" → ").append(FlowCore.sideLabel(FlowCore.DIM[t.dim],t.pick)).append("\n선택 모드: ").append(t.mode).append("\n사용 규칙: ").append(t.rule).append("\n현재 3모델 동등투표: + ").append(t.plusVotes).append(" / - ").append(t.minusVotes).append("\n방향: ").append(t.inverted?"역방향 재검증":"정방향 재검증");}else{ps.append("\n아직 탐색 준비중");}}patternDetail.setText(ps.toString());
        FlowCore.Backtest b=a.backtest;backtest.setText("학습 데이터 "+a.count+"/150회 · "+a.windowRange+"\n현재 최근흐름: "+a.suffix+"\n\n[AUTO FIND 선택방법 walk-forward]\n"+(t!=null&&t.certified?(t.validationN>0?t.validationHit+"/"+t.validationN+" = "+FlowCore.pct(t.validationRate)+"\n전반 "+FlowCore.pct(t.firstRate)+" · 최근 "+FlowCore.pct(t.secondRate):"초기표본 · 임시 강제추천") : "탐색 준비중")+"\n\n[기초 MHS 참고 재현]\n"+stat(b.globalHit,b.globalN)+"\n\n※ 현재 결과를 미리 보지 않고 과거 시점별로 순차검증");
        android.content.SharedPreferences sp=FlowCore.prefs(this);int n=sp.getInt(FlowCore.K_LIVE_TOTAL,0),hit=sp.getInt(FlowCore.K_LIVE_SUCCESS,0);double lp=Double.longBitsToDouble(sp.getLong(FlowCore.K_LIVE_PROFIT,Double.doubleToLongBits(0)));live.setText("실전 AUTO FIND · "+(n>0?hit+"/"+n+" = "+FlowCore.pct((double)hit/n)+" · 누적 "+FlowCore.signed(lp):"아직 없음"));int st=Math.max(5000,sp.getInt(FlowCore.K_BASE_STAKE,5000));double o=Math.max(1.01,sp.getFloat(FlowCore.K_ODDS,1.95f));int vh=t!=null&&t.certified?t.validationHit:0,vn=t!=null&&t.certified?t.validationN:0;double bt=vh*st*(o-1.0)-(vn-vh)*st;profit.setText("1회 배팅: "+FlowCore.money(st)+" · 배당 "+String.format(Locale.KOREA,"%.2f",o)+"\n적중 시: "+FlowCore.signed(st*(o-1.0))+" · 미적중 시: "+FlowCore.signed(-st)+"\n선택규칙 검증 가상손익: "+(vn>0?FlowCore.signed(bt):"-") );
    ...
    private void startExport(){Intent i=new Intent(Intent.ACTION_CREATE_DOCUMENT);i.addCategory(Intent.CATEGORY_OPENABLE);i.setType("application/json");i.putExtra(Intent.EXTRA_TITLE,"BubbleAutoFindV13_"+new SimpleDateFormat("yyyyMMdd_HHmm",Locale.KOREA).format(new Date())+".json");startActivityForResult(i,REQ_EXPORT);}
