...
public class MainActivity extends Activity{
    ...
    private View buildUi(){
        ...
        root.addView(tv("보글사다리 · TARGET 70 V1.2",24,Color.WHITE,true));TextView sub=tv("70% 목표 규칙 자동합성 · 미래값 가림 검증 · 최종1픽 1개",12,Color.rgb(110,231,183),false);sub.setPadding(0,dp(4),0,dp(14));root.addView(sub);
        ...
        LinearLayout ctrl=card();refresh=button("🔄 지금 수집 / 70% 규칙 생성",Color.rgb(5,150,105));ctrl.addView(refresh,new LinearLayout.LayoutParams(-1,dp(54)));background=new CheckBox(this);background.setText("백그라운드 자동추첨 ON");background.setTextColor(Color.WHITE);background.setTextSize(15);background.setPadding(0,dp(8),0,0);ctrl.addView(background);status=tv("조회 준비",12,Color.rgb(203,213,225),false);status.setPadding(0,dp(6),0,0);ctrl.addView(status);root.addView(ctrl);
        LinearLayout hero=card();hero.addView(tv("다음 회차 · TARGET 70 최종 1픽",12,Color.GRAY,false));nextRound=tv("-",15,Color.WHITE,true);hero.addView(nextRound);bestPick=tv("분석 대기",30,Color.rgb(52,211,153),true);bestPick.setPadding(0,dp(7),0,0);hero.addView(bestPick);bestConf=tv("검증 70%를 만든 규칙만 최종픽 사용",15,Color.rgb(253,224,71),true);bestConf.setPadding(0,dp(4),0,0);hero.addView(bestConf);TextView note=tv("최고값 선택 아님 · 서로 다른 3계열 규칙 합성 → 70% 달성 규칙만 채택",12,Color.rgb(110,231,183),true);note.setPadding(0,dp(10),0,0);hero.addView(note);root.addView(hero);
        LinearLayout dc=card();dc.addView(section("기초 MHS 3엔진 · 진단용"));dimSummary=tv("-",14,Color.WHITE,false);dimSummary.setLineSpacing(0,1.35f);dc.addView(dimSummary);root.addView(dc);
        LinearLayout pc=card();pc.addView(section("TARGET 70 합성엔진 상세"));patternDetail=tv("-",13,Color.rgb(226,232,240),false);patternDetail.setLineSpacing(0,1.3f);pc.addView(patternDetail);root.addView(pc);
        ...
        root.addView(tv("※ 70% 표시는 선택된 규칙의 과거 walk-forward 적중률입니다. 미래 적중을 보장하지 않으며, 70%를 만들지 못한 경우 최종픽을 확정하지 않습니다.",11,Color.GRAY,false));return sv;
    }
    ...
    private void manualSync(){
        saveSettingsSilent();refresh.setEnabled(false);status.setText("API 수집 + TARGET 70 규칙 합성/검증 중...");
    ...
    private void render(FlowCore.Analysis a,List<FlowCore.Result>d){if(a==null||d==null||d.isEmpty())return;FlowCore.Result last=d.get(0);nextRound.setText(last.round<480?last.date+" · "+(last.round+1)+"회":"다음날 · 1회");
        Target70Engine.Result t=a.target70;
        bestPick.setText(a.bestLabel);
        if(t!=null&&t.certified){bestConf.setText("🔥 TARGET 70 달성 · 검증 "+FlowCore.pct(t.validationRate)+" · "+t.validationHit+"/"+t.validationN);bestPick.setTextColor(Color.rgb(52,211,153));}
        else{bestConf.setText(t==null?"70% 규칙 생성 준비":"70% 미달 · 최종픽 확정 안 함 · "+FlowCore.pct(t.bestObservedRate));bestPick.setTextColor(Color.rgb(253,224,71));}
        ...
        StringBuilder ps=new StringBuilder();if(t==null){ps.append("TARGET 70 엔진 준비중");}else{ps.append(t.detail).append("\n\n검증한 합성규칙: ").append(t.searched).append("개");if(t.certified){ps.append("\n선택 항목: ").append(FlowCore.DIM[t.dim]).append(" → ").append(FlowCore.sideLabel(FlowCore.DIM[t.dim],t.pick)).append("\n규칙: ").append(t.rule).append("\n현재 3모델 투표: + ").append(t.plusVotes).append(" / - ").append(t.minusVotes).append("\n방향: ").append(t.inverted?"역방향 검증규칙":"정방향 검증규칙");}else{ps.append("\n현재 관측 최고 검증값(최종픽에 사용 안 함): ").append(FlowCore.pct(t.bestObservedRate)).append(" · ").append(t.bestObservedN).append("표본\n규칙: ").append(t.bestObservedRule).append("\n\n※ 70% 미달 후보는 최종픽으로 승격하지 않음");}}patternDetail.setText(ps.toString());
        FlowCore.Backtest b=a.backtest;backtest.setText("학습 데이터 "+a.count+"/150회 · "+a.windowRange+"\n현재 최근흐름: "+a.suffix+"\n\n[TARGET 70 선택규칙 walk-forward]\n"+(t!=null&&t.certified?t.validationHit+"/"+t.validationN+" = "+FlowCore.pct(t.validationRate)+"\n전반 "+FlowCore.pct(t.firstRate)+" · 후반 "+FlowCore.pct(t.secondRate):"아직 70% 달성 규칙 없음")+"\n\n[기초 MHS 참고 재현]\n"+stat(b.globalHit,b.globalN)+"\n\n※ 합성규칙은 각 과거 목표회차 이전 데이터만 사용해 검증");
        android.content.SharedPreferences sp=FlowCore.prefs(this);int n=sp.getInt(FlowCore.K_LIVE_TOTAL,0),hit=sp.getInt(FlowCore.K_LIVE_SUCCESS,0);double lp=Double.longBitsToDouble(sp.getLong(FlowCore.K_LIVE_PROFIT,Double.doubleToLongBits(0)));live.setText("실전 TARGET70 · "+(n>0?hit+"/"+n+" = "+FlowCore.pct((double)hit/n)+" · 누적 "+FlowCore.signed(lp):"아직 없음"));int st=Math.max(5000,sp.getInt(FlowCore.K_BASE_STAKE,5000));double o=Math.max(1.01,sp.getFloat(FlowCore.K_ODDS,1.95f));int vh=t!=null&&t.certified?t.validationHit:0,vn=t!=null&&t.certified?t.validationN:0;double bt=vh*st*(o-1.0)-(vn-vh)*st;profit.setText("1회 배팅: "+FlowCore.money(st)+" · 배당 "+String.format(Locale.KOREA,"%.2f",o)+"\n적중 시: "+FlowCore.signed(st*(o-1.0))+" · 미적중 시: "+FlowCore.signed(-st)+"\n선택규칙 검증 가상손익: "+(vn>0?FlowCore.signed(bt):"-") );
    ...
    private void startExport(){Intent i=new Intent(Intent.ACTION_CREATE_DOCUMENT);i.addCategory(Intent.CATEGORY_OPENABLE);i.setType("application/json");i.putExtra(Intent.EXTRA_TITLE,"BubbleTarget70V12_"+new SimpleDateFormat("yyyyMMdd_HHmm",Locale.KOREA).format(new Date())+".json");startActivityForResult(i,REQ_EXPORT);}
